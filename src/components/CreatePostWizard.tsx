...
const useAddPost = ({ userId }: { userId: string  }) => {
    const context = api.useContext();
    const session = useUser();

    return api.posts.create.useMutation({
        onMutate: async (newPost) => {
            await context.posts.getPostsByUserId.cancel();
            await context.posts.getPostByFollowers.cancel();

            const prevData = context.posts.getPostsByUserId.getData({
                userId,
            });
...
function CreatePostWizard({userId}:{userId:string}) {
    ...
    const { mutate: createPostMutation, isLoading } = useAddPost({ userId });
